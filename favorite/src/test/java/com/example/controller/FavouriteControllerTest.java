...
public class FavouriteControllerTest {
	...
	public void setUp() throws Exception {
		token=Jwts.builder().setId("Sam").setIssuedAt(new Date())
	...
	public void testGetAllFav() throws Exception {
		...
		mockMvc.perform(get("/api/v2/bookApp/getFavourites/Sam").header("Authorization","Bearer " +token))
...
//	
	...
	public void testAddBookToFavSuccess() throws Exception {
		...
		mockMvc.perform(post("/api/v2/bookApp/addToFavourites").header("Authorization","Bearer " +token)
		            .contentType(MediaType.APPLICATION_JSON)
		            .content(bookJson))
...
//	
	...
	public void testDeleteFav() throws Exception {
		...
		mockMvc.perform(delete("/api/v2/bookApp/deleteFromFavourite/1234/Sam").header("Authorization","Bearer " +token))
		//.andExpect(status().isOk())
//		.andExpect(jsonPath("$.name", is("Testing with Mockito")))
		.andDo(print());
	...
	public void testGetBYNameAndId() throws Exception  {
		...
		mockMvc.perform(get("/api/v2/bookApp/getFavouriteBooksByIdAndUsername/1234/Sam").header("Authorization","Bearer " +token))
		.andExpect(status().isOk())
//		.andExpect(jsonPath("$.name", is("Testing with Mockito")))
		.andDo(print());
