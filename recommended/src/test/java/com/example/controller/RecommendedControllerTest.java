...
public class RecommendedControllerTest {
	...
	public void setUp() throws Exception {
		 token=Jwts.builder().setId("Shreya").setIssuedAt(new Date())
	...
	public void testGetAllRecom() throws Exception {
		...
		mockMvc.perform(get("/api/v3/bookApp/getAllRecommendedBooks").header("Authorization","Bearer " +token))
...
//				.andExpect(jsonPath("$.name", is("Testing with Mockito")))
	...
	public void testGetRecomByIdAndName() throws Exception {
		when(service.getRecommendedBooksByIdAndUsername(Mockito.anyString(), Mockito.anyString())).
				thenReturn(listrecom);
		mockMvc.perform(get("/api/v3/bookApp/getRecommendedBooksByIdAndUsername/1234/shreya").header("Authorization","Bearer " +token))
				.andExpect(status().isOk())
//				.andExpect(jsonPath("$.name", is("Testing with Mockito")))
				.andDo(print());
	...
	public void testGetRecomByIdAndNameFailure() throws Exception {
		...
		mockMvc.perform(get("/api/v3/bookApp/getRecommendedBooksByIdAndUsername/1234/shreya").header("Authorization","Bearer " +token).contentType(MediaType.APPLICATION_JSON)
	...
	public void testGetRecomByName() throws Exception {
		when(service.getUsersAllRecommended(Mockito.anyString())).
				thenReturn(listrecom);
		mockMvc.perform(get("/api/v3/bookApp/getMyRecommendedBooks/shreya").header("Authorization","Bearer " +token))
				.andExpect(status().isOk())
//				.andExpect(jsonPath("$.name", is("Testing with Mockito")))
				.andDo(print());
...
//	}
	...
	public void testAddRecommendationSuccess() throws Exception{
		...
		//send a post request using mockMVC
		String recommendedJson = new ObjectMapper().writeValueAsString(recom);
		mockMvc.perform(post("/api/v3/bookApp/addToRecommendedBooks").header("Authorization","Bearer " +token)
						.contentType(MediaType.APPLICATION_JSON)
						.content(recommendedJson))
		.andExpect(status().isCreated());
		//verify mock has been called
	...
	public void testDeleteRecom() throws Exception {
		...
		mockMvc.perform(delete("/api/v3/bookApp/unrecommend/1234/shreya").header("Authorization","Bearer " +token))
		//.andExpect(status().isOk())
//		.andExpect(jsonPath("$.name", is("Testing with Mockito")))
		.andDo(print());
